...
- 在调用任何抓取工具前，先检查 `state/last_run.json.mcp_fetch_last_at`
  - 若 `now - mcp_fetch_last_at < 3 分钟`：本轮**禁止调用** `search_feeds/get_feed_detail`，直接跳至 Step 9，并在 growth.md 记录"触发抓取冷却"
...
- 本轮如果发生过实际抓取（调用了 `search_feeds`），在 Step 10 将 `mcp_fetch_last_at` 更新为当前时间（ISO8601）
